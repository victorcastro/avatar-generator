...
- The `Role / Position` label above the name field is dropped — the placeholder already says what goes in there — and its character counter moves under the input, right-aligned and quieter, 5px below it. The field keeps the label as an `aria-label` so it still has an accessible name.
