...
### Added
- A favicon: `assets/favicon.svg` for browsers that take one, a 32px PNG for those that do not, and a 180px `apple-touch-icon.png` on a solid square for the iOS home screen. The site had none. The mark is the avatar the tool makes — a full-bleed disc with a figure over a dark band — drawn as geometry rather than a downscaled render, so it survives 16px. The disc carries the brand red instead of the composition's near-black, which is what keeps it visible on a dark tab strip; the band below the figure stands in for the divider. A `theme-color` meta tag matches.
